...
const EXPIRIES_CSV       = 'https://docs.google.com/spreadsheets/d/e/2PACX-1vTXQVIPN42E20By0btiM2IFinhYkeNeYuz66b7bA5QEukcD_gLN-g7LGyArw05zaMJssbMxJm68DAkX/pub?gid=2030320336&single=true&output=csv'; // enabled
...
function parseCSV(text) {
  ...
  for (const line of lines) {
    // CSV split supporting quoted commas
...
// 3) Key mapping from Company Totals CSV
function normalizeKey(s){ return String(s||'').split(':')[0].trim().toLowerCase(); }
function mapCompanyTotals(rows) {
  const map = {};
  for (const r of rows) {
    if(!r || !r.length) continue;
    const keys = r.filter(c => isNaN(parseFloat(c))).map(normalizeKey).filter(Boolean);
    let value = 0;
...
function parseExpiries(rows){
  const out=[];
  for(let i=1;i<rows.length;i++){ const item=(rows[i][0]||'').trim(); const days=num(rows[i][1]); if(item) out.push({item,days}); }
  out.sort((a,b)=>a.days-b.days); return out;
}

// 5) Render
function getFirst(map, keys){ for(const k of keys){ const v=map[k.toLowerCase()]; if(v!==undefined) return v; } return 0; }
function renderKPIs(t){
  const totalBehaviour = getFirst(t,['total behaviour']);
  const totalIncidents = getFirst(t,['total incidents & accidents','total incidents and accidents','total incidents/accidents','total incidents','total hse events','total events']);
  ...
  document.getElementById('pb').textContent = fmt(getFirst(t,['positive behaviour/action','positive behaviour']));
  ...
  document.getElementById('pd').textContent = fmt(getFirst(t,['property or equipment damage']));
...
function renderCharts(t){
  ...
  incidentChart = new Chart(ic, {
    ...
    options:{ responsive:true, plugins:{ legend:{display:false}, title:{display:true,text:`Total Incidents & Accidents: ${fmt(totalIncidents)}` } } }
  });
...
function renderCompliance(list){
  ...
  list.forEach(({item,days})=>{ const [label,cls]=status(days);
